...
from dvc.commands.clearml.apply import CmdExperimentsApply
from dvc.commands.clearml.branch import CmdExperimentsBranch
from dvc.commands.clearml.clean import CmdExperimentsClean
from dvc.commands.clearml.diff import CmdExperimentsDiff
from dvc.commands.clearml.gc import CmdExperimentsGC
from dvc.commands.clearml.init import CmdExperimentsInit
from dvc.commands.clearml.ls import CmdExperimentsList
from dvc.commands.clearml.pull import CmdExperimentsPull
from dvc.commands.clearml.push import CmdExperimentsPush
from dvc.commands.clearml.remove import CmdExperimentsRemove
from dvc.commands.clearml.run import CmdExperimentsRun
from dvc.commands.clearml.save import CmdExperimentsSave
from dvc.commands.clearml.show import CmdExperimentsShow, show_experiments
...
def test_experiments_apply(dvc, scm, mocker):
    cli_args = parse_args(["clearml", "apply", "exp_rev"])
...
def test_experiments_run(dvc, scm, mocker):
    ...
    cmd = CmdExperimentsRun(parse_args(["cml", "run"]))
...
def test_experiments_branch(dvc, scm, mocker):
    cli_args = parse_args(["clearml", "branch", "expname", "branchname"])
...
def test_experiments_remove_invalid(dvc, scm, mocker, capsys, caplog):
    cmd = CmdExperimentsRemove(
        parse_args(["cml", "remove", "--all-commits", "exp-1", "exp-2"])
    )
    with pytest.raises(InvalidArgumentError):
        cmd.run()
...
def test_experiments_init(dvc, scm, mocker, capsys, extra_args):
    ...
    cli_args = parse_args(["clearml", "init", *extra_args, "cmd"])
    cmd = cli_args.func(cli_args)
...
def test_experiments_init_config(dvc, scm, mocker):
    ...
    m = mocker.patch("dvc.repo.experiments.init.init", return_value=(stage, [], []))
    cli_args = parse_args(["cml", "init", "cmd"])
    cmd = cli_args.func(cli_args)
...
def test_experiments_init_cmd_not_required_for_interactive_mode(dvc, mocker):
    cli_args = parse_args(["cml", "init", "--interactive"])
    cmd = cli_args.func(cli_args)
    assert isinstance(cmd, CmdExperimentsInit)
...
def test_experiments_init_extra_args(extra_args, expected_kw, mocker):
    cli_args = parse_args(["cml", "init", *extra_args, "cmd"])
    cmd = cli_args.func(cli_args)
    assert isinstance(cmd, CmdExperimentsInit)
...
def test_experiments_init_type_invalid_choice():
    with pytest.raises(DvcParserError):
        parse_args(["cml", "init", "--type=invalid", "cmd"])
...
def test_show_experiments_pcp(tmp_dir, mocker):
    ...
    experiments_table = mocker.patch("dvc.commands.clearml.show.experiments_table")
...
def test_experiments_save(dvc, scm, mocker):
    cli_args = parse_args(["cml", "save", "--name", "exp-name", "--force"])
...
def test_experiments_clean(dvc, scm, mocker):
    cli_args = parse_args(["clearml", "clean"])
